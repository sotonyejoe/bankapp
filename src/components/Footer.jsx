...
const Footer = () => (
    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        <div className="flex-1 flex flex-col justify-start mr-10">
          <img 
            src={logo}
            alt="hoobank"
            className="w-[266px] h-[72px] obkect-contain"
          />
            ...
            <div key={footerLink.title} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
              ...
              <ul className="list-none mt-4">
                {footerLink.links.map((link, index) => (
                  ...
                  hover:text-blue-300 cursor-pointer ${index !== footerLink.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                      {link.name}
                  </li>
                ))}
              </ul>
              <p>{footerLinks.length}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className='w-full flex justify-between items-center md:flex-row flex-col 
         border-t-[#3F3E45]  pt-6 border-t-[1px]'>
